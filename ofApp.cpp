	...
	for (int i = 0; i < image.getWidth(); i++) {
		for (int j = 0; j < image.getHeight(); j++) {
			...
			for (int k = 0; k < scene.size(); k++) {
				...
				closest = lambert(r.evalPoint(close), scene[c]->getNormal(glm::vec3(0,0,0)), closest, close);
				closest = phong(r.evalPoint(close), scene[c]->getNormal(glm::vec3(0, 0, 0)), closest, ofColor::white, p);
